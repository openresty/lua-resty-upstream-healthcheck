...
local function gen_upstream_prometheus_status(u, s, n)
    local r = string.format("nginx_upstream_status_info{name=\"%s\",status=\"%s\"} %d", u, s, n)
    return r
end
...
local function add_upstream_up_prometheus_status(tab, u)
    tab:add(gen_upstream_prometheus_status(u, "UP", 1))
    tab:add(gen_upstream_prometheus_status(u, "DOWN", 0))
    tab:add(gen_upstream_prometheus_status(u, "UNKNOWN", 0))
end

local function add_upstream_down_prometheus_status(tab, u)
    tab:add(gen_upstream_prometheus_status(u, "UP", 0))
    tab:add(gen_upstream_prometheus_status(u, "DOWN", 1))
    tab:add(gen_upstream_prometheus_status(u, "UNKNOWN", 0))
end
...
local function add_upstream_unknown_prometheus_status(tab, u)
    tab:add(gen_upstream_prometheus_status(u, "UP", 0))
    tab:add(gen_upstream_prometheus_status(u, "DOWN", 0))
    tab:add(gen_upstream_prometheus_status(u, "UNKNOWN", 1))
end

-- peer status generator functions

local function gen_peer_prometheus_status(u, p, r, s, n)
    local r = string.format("nginx_upstream_status_info{name=\"%s\",endpoint=\"%s\",status=\"%s\",role=\"%s\"} %d", u,
        p, s, r, n)
    return r
end
...
function _M.prometheus_status_page()
    ...
    stats_tab:add("# HELP nginx_upstream_status_info The running status of nginx upstream")
